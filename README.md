...
To overcome this problem, I wrote a very simple tool that scans all the assembly language files in a project, retrieves the declared data along with its type, and then analyzes the instructions that use this data and compares their types. 
If the types are different, it's likely a potential bug. 
The tool isn't very powerful in its parsing, but it allowed me to quickly potential bugs that would have been very complex to fix.
